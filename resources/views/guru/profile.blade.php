                    ...
                    <div class="profile-header">
                        <div class="row align-items-center">
                            <div class="col-auto profile-image">
                                <a href="#">
                                    <img class="rounded-circle" alt="{{ $guru->nama }}"
                                        src="{{ Storage::url('foto/' . $guru->foto) }}">
                                </a>
                            </div>
                            <div class="col ms-md-n2 profile-user-info">
                                <h4 class="user-name mb-0">{{ $guru->nama }}</h4>
                                <h6 class="text-muted">{{ $user->role }}</h6>
                                <div class="user-Location"><i class="fas fa-map-marker-alt"></i> {{ $guru->alamat }}</div>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <form action="{{ route('guru/profile/edit') }}" method="POST"
                                                ...
                                                <div class="row">
                                                    ...
                                                    <p id="no_hp" class="col-sm-9">{{ $guru->no_hp }}</p>
                                                </div>
                                                <div class="row">
                                                    <p class="col-sm-3 text-muted text-sm-end mb-0">Alamat</p>
                                                    <p id="alamat" class="col-sm-9 mb-0">{{ $guru->alamat }}</p>
                                                </div>
                                                <div class="row">
        ...
        <script>
            function editProfile() {
                ...
                document.getElementById('agama').innerHTML =
                    '<select class="form-control" name="agama"><option selected disabled>Silahkan pilih Agama </option><option value="Hindu" {{ $guru->agama == 'Hindu' ? 'selected' : '' }}>Hindu</option><option value="Kristen" {{ $guru->agama == 'Kristen' ? 'selected' : '' }}>Kristen</option><option value="Katolik" {{ $guru->agama == 'Katolik' ? 'selected' : '' }}>Katolik</option><option value="Islam" {{ $guru->agama == 'Islam' ? 'selected' : '' }}>Islam</option><option value="Budha" {{ $guru->agama == 'Budha' ? 'selected' : '' }}>Budha</option></select>';
                ...
                document.getElementById('jenis_kelamin').innerHTML =
                    '<select class="form-control" name="jenis_kelamin"><option selected disabled>Select Jenis Kelamin</option><option value="P" {{ $guru->jenis_kelamin == 'P' ? 'selected' : 'P' }}>Perempuan</option><option value="L" {{ $guru->jenis_kelamin == 'L' ? 'selected' : '' }}>Laki-Laki</option></select>';
            ...
            function cancelEdit() {
                ...
                document.getElementById('nama').innerHTML = '{{ $guru->nama }}';
                document.getElementById('agama').innerHTML = '{{ $guru->agama }}';
                document.getElementById('tanggal_lahir').innerHTML = '{{ $guru->tanggal_lahir }}';
                document.getElementById('tempat_lahir').innerHTML = '{{ $guru->tempat_lahir }}';
                document.getElementById('jenis_kelamin').innerHTML =
                    @if ($guru->jenis_kelamin == 'L')
                        'Laki-Laki'
                    @else
                ...
                document.getElementById('no_hp').innerHTML = '{{ $guru->no_hp }}';
                document.getElementById('alamat').innerHTML = '{{ $guru->alamat }}';
                document.getElementById('foto').innerHTML = '{{ $guru->foto }}';
            ...
            function editTentang() {
                document.getElementById('tentang').innerHTML =
                    '<textarea id="tentang" name="tentang" rows="10" cols="20">{{ $guru->tentang }}</textarea>';
                document.getElementById('edit-about').style.display = 'block';
            }

            function cancelTentang() {
                document.getElementById('tentang').innerHTML = '{{ $guru->tentang }}';
