...
@section('content')
                    ...
                    <div class="profile-header">
                        <div class="row align-items-center">
                            <div class="col-auto profile-image">
                                <a href="#">
                                    <img class="rounded-circle" alt="{{ $siswa->nama_siswa }}"
                                        src="{{ Storage::url('foto-siswa/' . $siswa->foto) }}">
                                </a>
                            </div>
                            <div class="col ms-md-n2 profile-user-info">
                                <h4 class="user-name mb-0">{{ $siswa->nama_siswa }}</h4>
                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            <h5 class="card-title d-flex justify-content-between">
                                                ...
                                                <a class="edit-link" data-bs-toggle="modal" href="#"
                                                    onclick="editProfile()"><i class="far fa-edit me-1"></i>Edit</a>
                                            </h5>
                                            <form action="{{ route('siswa/profile/edit') }}" method="post"
                                                ...
                                                <div class="row">
                                                    ...
                                                    <p id="nama_siswa" class="col-sm-9">{{ $siswa->nama_siswa }}</p>
                                                </div>
                                                <div class="row">
    ...
    <script>
        function editProfile() {
            ...
            document.getElementById('nama_siswa').innerHTML =
                '<input type="text" name="nama_siswa" value="{{ $siswa->nama_siswa }}" class="form-control">';
            document.getElementById('kelas').innerHTML =
                '<select class="form-control"name="kelas"><option selected disabled>Silahkan pilih Kelas </option>@foreach ($kelass as $kelas)<option value="{{ $kelas->id }}"{{ $siswa->kelas_id == $kelas->id ? 'selected' : '' }}>{{ $kelas->nama }}</option>@endforeach</select>';
            document.getElementById('jurusan').innerHTML =
                '<select class="form-control"name="jurusan"><option selected disabled>Silahkan pilih Jurusan </option>@foreach ($jurusans as $jurusan)<option value="{{ $jurusan->id }}"{{ $siswa->jurusan_id == $jurusan->id ? 'selected' : '' }}>{{ $jurusan->nama }}</option>@endforeach</select>';
        ...
        function cancelEdit() {
            ...
            document.getElementById('nama_siswa').innerHTML = '{{ $siswa->nama_siswa }}';
            document.getElementById('kelas').innerHTML = '{{ $siswa->kelas->nama }}';
            document.getElementById('jurusan').innerHTML = '{{ $siswa->jurusan->nama }}';
